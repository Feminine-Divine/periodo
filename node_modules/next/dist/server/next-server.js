...
class NextNodeServer extends _baseServer.default {
    ...
    compression = this.nextConfig.compress ? (0, _compression).default() : undefined;
    ...
    getFontLoaderManifest() {
        ...
        return require((0, _path).join(this.distDir, "server", `${_constants.FONT_LOADER_MANIFEST}.json`));
    }
    getFallback(page) {
   ...
   */ async handleApiRequest(req, res, pathname, query) {
        ...
        if (!pageFound && this.dynamicRoutes) {
            for (const dynamicRoute of this.dynamicRoutes){
                ...
                if ((0, _isApiRoute).isAPIRoute(dynamicRoute.page) && params) {
        ...
        if (!result) {
            ...
            for (let [key, value] of allHeaders){
                ...
                if (key.toLowerCase() === "set-cookie") {
                    (0, _requestMeta).addRequestMeta(params.request, "_nextMiddlewareCookie", (0, _utils2).splitCookiesString(value));
                }
            }
        }
        return result;
